...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=None, patch_size=[8,8], in_c=13, embed_dim=768, norm_layer=None):
...
class AFNO(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.bias:
            bias = self.bias(x.permute(0, 2, 1).contiguous()).permute(0, 2, 1).contiguous()
        else:
...
class Fblock(nn.Module):
    ...
    def forward(self, x):
        ...
        #x = x + self.drop_path(self.attn(self.filter(self.normlayer1(x))))
...
class GF_Block(nn.Module):
    def __init__(self,
        ...
        self.blks = nn.ModuleList([Fblock(
            dim=embed_dim,
            mlp_ratio=mlp_ratio,
            drop=drop_rate,
            drop_path=dpr[i],
        ...
        self.lproj = nn.Sequential(OrderedDict([
            ...
            ('act2', nn.Tanh()),
            ('transposeconv3', nn.ConvTranspose2d(out_channels * 4, out_channels, kernel_size=(int(patch_size[0]/4),int(patch_size[1]/4)), stride=(int(patch_size[0]/4),int(patch_size[1]/4))))
        ]))
...
class FM(nn.Module):
    def __init__(self, shape_in, dS=512, dT=256, N_S=4, N_T=8, incep_ker=[3,5,7,11], groups=8):
