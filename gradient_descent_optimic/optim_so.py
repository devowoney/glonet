...
DEFAULT_OUTPUT_DIR = f"/Odyssey/private/j25lee/glonet/gradient_descent_optimic/outputs/man_optimIC_GD/{now}_so"
...
class ManualGradientDescent:
    ...
    def load_data(self) -> None:
        ...
        self.x0_1_s = self.x0_1[:, :, 2:3, :, :].clone().detach()
        self.x0_2_s = self.x0_2[:, :, 10:20, :, :].clone().detach()
        self.x0_3_s = self.x0_3[:, :, 10:20, :, :].clone().detach()
        self.x0_1_s.requires_grad = True
        self.x0_2_s.requires_grad = True
        self.x0_3_s.requires_grad = True

        self.x0_1_left_1 = self.x0_1[:, :, 0:2, :, :].clone().detach()
        self.x0_1_left_2 = self.x0_1[:, :, 3:5, :, :].clone().detach()
        self.x0_2_left_1 = self.x0_2[:, :, 0:10, :, :].clone().detach()
        self.x0_2_left_2 = self.x0_2[:, :, 20:40, :, :].clone().detach()
        self.x0_3_left_1 = self.x0_3[:, :, 0:10, :, :].clone().detach()
        self.x0_3_left_2 = self.x0_3[:, :, 20:40, :, :].clone().detach()
        ...
        self.target_1_s = self.target1[:, 2:3, :, :]
        self.target_2_s = self.target2[:, 10:20, :, :]
        self.target_3_s = self.target3[:, 10:20, :, :]
        
        self.ocean_mask_1_s = self.ocean_mask_1[2:3, :, :]  # Only SSH channel
        self.ocean_mask_2_s = self.ocean_mask_2[10:20, :, :]
        self.ocean_mask_3_s = self.ocean_mask_3[10:20, :, :]
        
        log.info(f"Loaded data shapes:")
        log.info(f"  so (x_1): {self.x0_1.shape}, Target for so: {self.target_1_s.shape}")
        log.info(f"  so (x_2): {self.x0_2.shape}, Target for so: {self.target_2_s.shape}")
        log.info(f"  so (x_3): {self.x0_3.shape}, Target for so: {self.target_3_s.shape}")
        log.info(f"  Gradient tracking only for so")
    ...
    def forward(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        with torch.enable_grad():
            ...
            self.x0_1_combined = torch.cat([self.x0_1_left_1, self.x0_1_s, self.x0_1_left_2], dim=2) 
            self.x1 = self.normalizer1(self.x0_1_combined)
            self.x0_2_combined = torch.cat([self.x0_2_left_1, self.x0_2_s, self.x0_2_left_2], dim=2)
            self.x2 = self.normalizer2(self.x0_2_combined)
            self.x0_3_combined = torch.cat([self.x0_3_left_1, self.x0_3_s, self.x0_3_left_2], dim=2)
        ...
        with torch.enable_grad():
            ...
            y_hat1 = y_hat1[:, 2:3, :, :]  # Retrive only salinity channel
            y_hat2 = y_hat2[:, 10:20, :, :]
            y_hat3 = y_hat3[:, 10:20, :, :]
    ...
    def compute_loss(self, y_hat1: torch.Tensor, y_hat2: torch.Tensor, y_hat3: torch.Tensor) -> torch.Tensor:
        ...
        loss1 = self.loss_fn(y_hat1, self.target_1_s)
        loss2 = self.loss_fn(y_hat2, self.target_2_s)
        loss3 = self.loss_fn(y_hat3, self.target_3_s)
    ...
    def compute_variable_errors(self, y_hat1: torch.Tensor, y_hat2: torch.Tensor, y_hat3: torch.Tensor) -> Dict:
        ...
        with torch.no_grad():
            ...
            y_all = torch.cat([self.target_1_s, self.target_2_s, self.target_3_s], dim=1) # [1, 21, H, W]
            ...
            ocean_mask_all = torch.cat([self.ocean_mask_1_s, self.ocean_mask_2_s, self.ocean_mask_3_s], dim=0).unsqueeze(0)      
            # [1, 21, H, W]
    ...
    def optimize(self) -> Dict:
        ...
        for iteration in range(self.num_iterations):
            ...
            if self.x0_1_s.grad is not None:
                self.x0_1_s.grad.zero_()
            if self.x0_2_s.grad is not None:
                self.x0_2_s.grad.zero_()
            if self.x0_3_s.grad is not None:
                self.x0_3_s.grad.zero_()    
            ...
            with torch.no_grad():
                ...
                if self.x0_1_s.grad is not None:
                    self.x0_1_s.grad *= self.ocean_mask_1_s.unsqueeze(0).unsqueeze(0)
                if self.x0_2_s.grad is not None:
                    self.x0_2_s.grad *= self.ocean_mask_2_s.unsqueeze(0).unsqueeze(0)
                if self.x0_3_s.grad is not None:
                    self.x0_3_s.grad *= self.ocean_mask_3_s.unsqueeze(0).unsqueeze(0)
            ...
            with torch.no_grad():
                ...
                if self.x0_1_s.grad is not None:
                    self.x0_1_s -= self.learning_rate * self.x0_1_s.grad
                if self.x0_2_s.grad is not None:
                    self.x0_2_s -= self.learning_rate * self.x0_2_s.grad
                if self.x0_3_s.grad is not None:
                    self.x0_3_s -= self.learning_rate * self.x0_3_s.grad
            ...
            grad_norm_s_1 = self.x0_1_s.grad.norm().item() if self.x0_1_s.grad is not None else 0.0
            grad_norm_s_2 = self.x0_2_s.grad.norm().item() if self.x0_2_s.grad is not None else 0.0
            grad_norm_s_3 = self.x0_3_s.grad.norm().item() if self.x0_3_s.grad is not None else 0.0
            ...
            if (iteration + 1) % 10 == 0 or iteration == 0:
                ...
                log.info(f"  Grad Norm - x0s1: {grad_norm_s_1:.6f}, Grad Norm - x0s2: {grad_norm_s_2:.6f},"
                         f"Grad Norm - x0s3: {grad_norm_s_3:.6f}")
                log.info("-" * 60)
