...
sys.path.insert(1, '/Odyssey/private/j25lee/glonet/glonet_daily_forecast_local')
...
def setup_slurm_task():
    ...
    if 'SLURM_PROCID' in os.environ:
        ...
        # Get number of GPUs visible to THIS process (respects CUDA_VISIBLE_DEVICES)
        num_gpus = torch.cuda.device_count()
        cuda_visible = os.environ.get('CUDA_VISIBLE_DEVICES', 'not set')
        
        logger.info(f"[Task {rank}] CUDA_VISIBLE_DEVICES={cuda_visible}, visible GPUs={num_gpus}")
        
        if num_gpus == 0:
            raise RuntimeError("No GPUs available")
        
        # Use SLURM_LOCALID for node-local task ID (best for multi-GPU per node)
        if 'SLURM_LOCALID' in os.environ:
            local_id = int(os.environ['SLURM_LOCALID'])
            # If we have multiple GPUs visible, use local_id to select among them
            local_rank = local_id % num_gpus
            local_rank = 0 # GLONET explicitly using cuda:0
        else:
            # If only 1 GPU is visible to this process, use it (device 0)
            local_rank = 0
        
        # Set GPU device for this task
        torch.cuda.set_device(local_rank)
        
        logger.info(f"[Task {rank}/{world_size}] Using local GPU index {local_rank}")
...
def aforecast_batch(d_batch, dates, cycle: int, device):
    ...
    for i in range(1, int((cycle + 1) / 2) + 1):
        ...
        for b in range(batch_size):
            ...
            d = denormalizer(outvar_cpu[b:b+1])
...
def create_forecast_batch(member_files: List[Path],
    ...
    # Run batch forecasts
    ts_list1_batch = aforecast_batch(rdata1_batch, dates, cycle=forecast_cycle, device=device)
    del rdata1_batch
    gc.collect()
    
    ts_list2_batch = aforecast2_batch(rdata2_batch, dates, cycle=forecast_cycle, device=device)
    del rdata2_batch
    gc.collect()
    
    ts_list3_batch = aforecast3_batch(rdata3_batch, dates, cycle=forecast_cycle, device=device)
    del rdata3_batch
    gc.collect()
    ...
    for b in range(batch_size):
        # ds1_batch[b] is a list of tensors (one per time step)
        # Concatenate time steps along time dimension (dim=1)
        tensor1 = torch.cat(ts_list1_batch[b], dim=1)  # [1, time, ch, lat, lon]
        tensor2 = torch.cat(ts_list2_batch[b], dim=1)
        tensor3 = torch.cat(ts_list3_batch[b], dim=1)
        
        # Concatenate the three parts along channel dimension (dim=2)
        combined_tensor = torch.cat([tensor1, tensor2, tensor3], dim=2)  # [1, time, 85, lat, lon]
        ...
        del tensor1, tensor2, tensor3, combined_tensor
        gc.collect()
    
    del ts_list1_batch, ts_list2_batch, ts_list3_batch
    gc.collect()
